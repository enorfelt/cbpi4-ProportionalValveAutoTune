...
@parameters([
    ...
    Property.Number(label = "Output_Step", configurable = True, default_value = 100, description="Default: 100. Sets the output when stepping up/down."),
...
class PropValveAutoTune(CBPiActor):
    ...
    async def on_start(self):
        ...
        self.pumpActor = self.props.get("PumpActor", None) 
        pass
    ...
    async def run(self):
        ...
        current_value=self.get_sensor_value(self.volumeSensor).get("value")

        if setpoint == None:
            self.cbpi.notify('PID AutoTune', 'You have not defined a target volume. System will set target to {} L and start AutoTune'.format(15), NotificationType.WARNING)
            setpoint=15
            await self.settarget(setpoint)
    
        if setpoint < current_value:
            self.cbpi.notify('PID AutoTune', 'Your target volume is above the current volume. Choose a higher setpoint or wait until volume is below target volume and restart AutoTune', NotificationType.ERROR)
        ...
        lookbackSec = float(self.props.get("lockback_seconds", 10))
        heat_percent_old = 0
        try:
